...
module.exports = class SelectionManager {
  constructor() {
    ...
    this.editorSubscriptions.add(
      lumine.workspace.onWillDestroyPaneItem((item) => {
        if (lumine.workspace.isTextEditor(item.item)) {
    ...
    this.activeItemSubscription = lumine.workspace.onDidChangeActivePaneItem(() => {
  ...
  // Public: the highlight markers currently placed in the given editor.
  ...
  debouncedHandleSelection() {
    if (!this.handleSelectionDebounce) {
      ...
      }, lumine.config.get("highlight-selected.timeout"));
    }
    this.handleSelectionDebounce();
  }

  listenForTimeoutChange() {
    this.timeoutSubscription = lumine.config.onDidChange("highlight-selected.timeout", () => {
      if (this.handleSelectionDebounce) {
        this.handleSelectionDebounce.clear();
      }
      this.handleSelectionDebounce = null;
